...
2. **跨内核子会话**：delegate_agent 可选 `agentRuntime="pi"`，子会话跑 Pi Agent Runtime（适合 OpenAI Responses 等非 Anthropic 协议模型）
3. **Pro 数据隔离**：业务数据 `~/.proma-pro`、electron userData `@proma/electron-pro`，与原版完全隔离，可同时运行
4. **绿色图标**：托盘 + exe 图标替换为绿色，区分 Pro 版和原版

## 前置条件

- **Windows x64**（rcedit、skia 原生模块均绑定 win32-x64-msvc）
- **Node.js v22+**（推荐 `C:/Program Files/nodejs/node.exe`）
- **Proma 官方安装版**（默认从 `C:/Users/<user>/AppData/Local/Programs/Proma` 读取 app.asar）
...
├── scripts/
│   ├── patch-main.cjs                # 核心补丁脚本（跨渠道 + 跨内核 + 数据隔离 + 图标）
